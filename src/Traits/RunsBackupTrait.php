...
/**
 * Shared create -> upload -> report flow for the backup commands.
 *
 ...
 * up" skip and reporting live here once. (The heartbeat success timestamp is
 * stamped by the upload services themselves, so trigger-driven backups record
 * it too.) Requires the using class to also use {@see RendersReportTrait} and
 * {@see DisplayHelperTrait} and to be an {@see \Illuminate\Console\Command}.
 */
trait RunsBackupTrait
{
     ...
     */
    protected function runBackup(string $label, Closure $create, Closure $send): int
    {
